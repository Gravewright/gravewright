...
(() => {
  ...
  function docIsEmpty(doc) {
    const content = doc && doc.doc && doc.doc.content;
  ...
  function mountBlockEditorsIn(root) {
    whenBlockEditorReady(() => {
      root.querySelectorAll("[data-journal-block-editor]").forEach((host) => {
        ...
        const legacyMd = readJsonScript(host, "[data-journal-doc-markdown]");
        ...
        if (docIsEmpty(doc) && typeof legacyMd === "string" && legacyMd.trim() && window.marked) {
          const html = window.DOMPurify
            ? window.DOMPurify.sanitize(window.marked.parse(legacyMd))
            : window.marked.parse(legacyMd);
          controller.setContentFromHTML(html);
        }
