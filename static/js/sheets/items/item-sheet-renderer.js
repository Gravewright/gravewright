...
(function () {
  ...
  function htmlSheetData(bundle) {
    ...
    return {
      item: { id: bundle.item?.id, name: bundle.item?.name, type: bundle.item?.type, ...data },
      system: data,
  ...
  function writeHtmlSheetPath(root, path, value) {
    ...
    if (target === "item.name" || target === "core.name") target = "core.name";
    else if (target.startsWith("system.")) target = "sheet." + target.slice("system.".length);
    else if (target.startsWith("item.")) target = "sheet." + target.slice("item.".length);
    writePath(root, target, value);
  }

  async function renderHtmlSheet(root, bundle) {
    const HTML = window.GravewrightHTMLSheets;
    const packageId = bundle.item?.system_id || "";
    const sheetType = bundle.item?.type || "";
