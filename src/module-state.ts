...
/** Reads and atomically persists module activation states. */
...
/** Returns a stable key order so persisted state produces deterministic diffs. */
...
/** Creates a serialized, atomic state store backed by a JSON file. */
    ...
    set(name, state) {
      ...
      const persist = async (): Promise<void> => {
        ...
        try {
          ...
          try { await operations.unlink(temporaryPath); } catch { /* Best-effort cleanup preserves the original write error. */ }
          throw error;
        }
        states = nextStates;
      };
