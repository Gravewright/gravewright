...
Examples:
  ...
  grave new module fog-of-war --example-complete
  grave doctor`;
...
Checks project state, concrete dependencies and the required server, frontend and backend.`,
...
/** Injects the working directory and output adapter used by the CLI entry point. */
export interface MainOptions { cwd?: string; output?: CliOutput; }

/** Formats an unknown-command error and selects the CLI usage exit code. */
...
/** Lists active modules that opted into an administrative operation. */
...
/** Resolves a project root and reports a user-facing error when none exists. */
...
/** Parses CLI arguments, runs one command, and returns its process exit code. */
