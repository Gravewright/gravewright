...
> [!WARNING]
> Gravewright é software Beta. A SDK 1 RC 1 é o candidato público congelado de
> compatibilidade. Packages declaram `sdkVersion: "1"` e devem usar
> `compatibility: { "minimum": "1", "verified": "1" }`; RC é metadado de
> release e não altera esses campos.
...
- [`portando-modulos.md`](portando-modulos.md): processo completo para adaptar módulos de outras plataformas ao Gravewright.
...
- [`hardening-de-pacotes.md`](hardening-de-pacotes.md): validação de paths, ZIPs, assets e autorização operacional.
