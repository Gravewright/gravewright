...
=======
installing, enabling, and running across `1.x` releases — unless it uses APIs
explicitly marked `experimental`, `legacy_experimental`, or `deprecated`.
>>>>>>> origin/main
...
=======
| `experimental` | May change between releases; must emit a warning and/or be documented as experimental. |
| `legacy_experimental` | Existing useful API that will **not** be promoted to a definitive contract. |
| `deprecated` | Still works, but will be removed or replaced. |
| `internal` | Internal use. Packages must not depend on it. |
>>>>>>> origin/main
...
=======
| `hooks.client` (`sdk.hooks.*`, `sdk.events.*`) | `legacy_experimental` | Not the formal interop contract. Superseded by `sdk.bus.*`. |
| `storage.sqlite` (`sdk.storage.sqlite.*`) | `experimental` | Contract in Phase 7A, runtime in Phase 7B. |
| `sdk.bus.*` (`bus.publish/subscribe/request/provide`) | `experimental` | Formal interop, Phase 12. Must not be a wrapper over hooks. |
| `sheets.html` / `sheets.controller` | planned | Phase 13; not implemented before the base is stable. |

## Policy requirements

1. Every public API has a status (`stable`, `experimental`, `legacy_experimental`,
   `deprecated`, or `internal`). No public API may be statusless.
2. Experimental APIs appear as such in documentation and/or the manifest/doctor.
3. The doctor can warn about deprecated, experimental, or legacy capabilities per
   the configured policy.
4. Breaking changes to the manifest contract require a new `sdkVersion` or a
   formal migration.
5. Capabilities are declared in the canonical `capabilities.json` (Phase 2);
   every public, gated method maps to a capability there.
...
=======
| `v1.0.0-alpha.5` | `sdk.bus.*` interop experimental. |
>>>>>>> origin/main
