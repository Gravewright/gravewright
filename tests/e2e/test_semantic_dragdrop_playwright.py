...
PACKAGE_ID="semantic-dragdrop-e2e"
@pytest.fixture(scope="module")
def scene_runtime_server(tmp_path_factory):
    tmp=tmp_path_factory.mktemp("semantic-dragdrop-e2e");db_path=tmp/"e2e.sqlite3";seeded=_seed_database(db_path);data=tmp/"data";target=data/"packages/addons"/PACKAGE_ID;target.parent.mkdir(parents=True);shutil.copytree(REPO_ROOT/"tests/fixtures/sdk_packages/valid/addons"/PACKAGE_ID,target);text=(target/"manifest.json").read_text(encoding="utf-8");manifest=json.loads(text);ruleset=data/"packages/rulesets/dnd5e";ruleset.parent.mkdir(parents=True);shutil.copytree(REPO_ROOT/"data/packages/rulesets/dnd5e",ruleset);rules_manifest=json.loads((ruleset/"manifest.json").read_text(encoding="utf-8"));rules_manifest["provides"]["rules"]["itemInsertion"]="rules/item-insertion.gw.json";(ruleset/"manifest.json").write_text(json.dumps(rules_manifest),encoding="utf-8");(ruleset/"rules/item-insertion.gw.json").write_text(json.dumps({"slots":[{"id":"inventory","storage":"inventory","accepts":["equipment"],"duplicatePolicy":"allow","mapping":{"name":"core.name","type":"core.type"}}]}),encoding="utf-8");rules_text=(ruleset/"manifest.json").read_text(encoding="utf-8")
    ...
    db_module.DATABASE_PATH=db_path.resolve();db_module._initialized=False;engine_module.reset_engine();scene=seed_scene(seeded["campaign_id"]);SceneRepository().set_active_scene(campaign_id=seeded["campaign_id"],scene_id=scene["id"]);seeded["scene_id"]=scene["id"];seeded["actor_id"]=ActorRepository().create(campaign_id=seeded["campaign_id"],system_id="dnd5e",actor_type="character",name="Drop Actor",created_by_user_id=seeded["gm_id"]);seeded["item_id"]=ItemRepository().create(campaign_id=seeded["campaign_id"],system_id="dnd5e",item_type="equipment",name="Drop Rope",created_by_user_id=seeded["gm_id"])
    InstalledPackageRepository().upsert(package_id="dnd5e",kind="ruleset",name=rules_manifest["name"],version=rules_manifest["version"],status="enabled",package_dir="rulesets/dnd5e",manifest_json=rules_text,compatibility_status="compatible",validation_errors_json="[]",installed_by_user_id=seeded["gm_id"],last_validation_status="valid");CampaignRepository().update_system(campaign_id=seeded["campaign_id"],changed_by_user_id=seeded["gm_id"],next_system_id="dnd5e")
