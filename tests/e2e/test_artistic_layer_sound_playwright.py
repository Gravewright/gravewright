...
def test_artistic_layer_places_and_restores_spatial_sound(sound_server):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.locator('[data-open-sound-modal="effect"]').click();effect_modal=page.locator('[data-sound-product-modal="effect"]:not([hidden])');expect(effect_modal).to_be_visible();expect(effect_modal.locator(".sound-window-toolbar")).to_have_count(0);expect(effect_modal.get_by_text("Novo efeito sonoro",exact=True)).to_be_visible();effect_modal.locator("[data-place-spatial-sound]").click();expect(page.locator("[data-spatial-placement-hint]")).to_be_visible();expect(page.locator("[data-spatial-placement-hint]")).to_contain_text("Clique e arraste no mapa")
            ...
            inspector=page.locator(".spatial-sound-inspector:not([hidden])");expect(inspector).to_be_visible();expect(page.locator("[data-spatial-placement-hint]")).to_be_hidden();expect(inspector.locator('[data-spatial-sound-source]')).to_have_value("Rain");expect(inspector.locator('[name="radius"]')).not_to_have_value("350.0");expect(inspector.locator('[name="constrainedByWalls"]')).to_be_checked();expect(inspector.get_by_text("Tokens fora deste raio",exact=False)).to_be_visible()
...
def test_core_area_markers_and_game_only_hand_icon(sound_server):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            dock.locator('[data-tool="shape"]').click();shape_panel=page.locator('[data-tool-sub-panel="shape"]:not([hidden])');buttons=shape_panel.locator("[data-area-marker-preset]");expect(buttons).to_have_count(4);assert buttons.all_text_contents()==["Linha","Círculo","Quadrado","Cone"]
...
def test_area_marker_can_move_rotate_delete_and_clear(sound_server):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            dock.locator('[data-tool="shape"]').click();panel=page.locator('[data-tool-sub-panel="shape"]:not([hidden])');panel.locator('[data-area-marker-preset]').filter(has_text="Quadrado").click()
