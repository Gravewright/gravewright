...
class GmGuidedPrefetchBroker:
    ...
    def record_player_viewport(
        self, *, user_id: str, scene_id: str, cx0: int, cy0: int, cx1: int, cy1: int
    ) -> None:
        now_ms = self._clock_ms()
        with self._lock:
            self._players[(scene_id, user_id)] = PlayerViewport(
                user_id, scene_id, cx0, cy0, cx1, cy1, now_ms
            )
            self._prune(now_ms)
        ...
        *,
        ...
        with self._lock:
            ...
            if len(self._observations) > 128:
                oldest = min(
                    self._observations,
                    key=lambda key: self._observations[key].last_seen_at_ms,
                )
            ...
            observation.camera_deceleration = max(
                0.0, float(camera_deceleration), observation.camera_speed - speed
            )
            observation.camera_speed = speed
            observation.camera_velocity_ema = self._ewma(
                speed, observation.camera_velocity_ema
            )
            observation.interaction_count = min(
                3, observation.interaction_count + max(0, int(interaction_count))
            )
            ...
            for (player_scene_id, player_id), viewport in self._players.items():
                ...
                if distance > self.max_distance_chunks:
                    continue
                signal = observation.player_signals.setdefault(
                    player_id, _PlayerSignal()
                )
                distance_score = (
                    1.0 / (1.0 + distance) if self.policy == "simple" else 1.0
                )
                revisit_boost = min(1.25, 1.0 + 0.10 * observation.revisit_count)
                interaction_boost = 1.0 + 0.08 * observation.interaction_count
                deceleration_boost = 1.0 + 0.08 * min(
                    observation.camera_deceleration, 2.0
                )
                ...
                signal.confidence_ema = (
                    raw_confidence
                    if self.policy == "simple"
                    else self._ewma(raw_confidence, previous_ema)
                )
                signal.previous_distance_ema = signal.distance_ema
                signal.distance_ema = (
                    float(distance)
                    if signal.distance_ema is None
                    else self._ewma(float(distance), signal.distance_ema)
                )
                ...
                foundation = signal.confidence_ema >= min(
                    self.keep_threshold, threshold * self.confidence_alpha
                )
                ...
                if (
                    ...
                    and now_ms - last_emitted < self.repeat_after_ms
                ):
                    continue
                ...
                hints.append(
                    GmPrefetchHint(
                        ...
                        cy1=cy1,
                        score=score,
                        confidence=signal.confidence_ema,
                        momentum=momentum,
                        utility=score,
                        policy=self.policy,
                        dwell_ms=dwell_ms,
                        distance_chunks=distance,
                        expires_at_ms=now_ms + self.hint_ttl_ms,
                        revisit_count=observation.revisit_count,
                        interaction_count=observation.interaction_count,
                        camera_speed=observation.camera_velocity_ema,
                        camera_deceleration=observation.camera_deceleration,
    ...
    def _intent_score(self, confidence: float, momentum: float) -> float:
        if self.policy in {"sigmoid_derivative", "utility_per_byte"}:
            ...
            return clamp(
                ...
                + self.momentum_beta * normalized_momentum,
                0.0,
                1.0,
            )
        return confidence
    ...
    def _momentum(self, signal: _PlayerSignal, now_ms: int) -> float:
        ...
        if dt_ms <= 0 or dt_ms > self.max_derivative_interval_ms:
            return 0.0
        derivative = (signal.confidence_ema - signal.previous_confidence_ema) / (
            dt_ms / 1_000
        )
        return clamp(
            derivative, -self.max_momentum_per_second, self.max_momentum_per_second
        )
    ...
    @staticmethod
    def _rect_distance(
        viewport: PlayerViewport, cx0: int, cy0: int, cx1: int, cy1: int
    ) -> int:
    ...
    def _prune(self, now_ms: int) -> None:
        self._players = {
            ...
            if now_ms - value.observed_at_ms <= self.viewport_ttl_ms
        }
        observation_ttl = max(self.hint_ttl_ms, self.repeat_after_ms)
        self._observations = {
            key: value
            for key, value in self._observations.items()
            if now_ms - value.last_seen_at_ms <= observation_ttl
        }
        active = {(key[0], key[1]) for key in self._observations}
        self._last_region_by_gm = {
            key: value
            for key, value in self._last_region_by_gm.items()
            if key in active
        }
