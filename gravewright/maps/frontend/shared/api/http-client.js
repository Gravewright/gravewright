...
export { ApiError, TransportError } from "./api-error.js";
/** How long an ordinary call waits before the server counts as unreachable. */
...
/**
 * The server's protocol: JSON in, JSON out, a CSRF token in front of everything that changes
 * state, and a refusal turned into an `ApiError` carrying the domain's own code. It owns no
 * mechanism — the exchange belongs to whichever `HttpTransport` it was given.
 */
export class HttpClient {
    ...
    post(path, body, options) { return this.#guarded("POST", path, body, options); }
    patch(path, body, options) { return this.#guarded("PATCH", path, body, options); }
    delete(path, body, options) { return this.#guarded("DELETE", path, body, options); }
    /**
    ...
    async #send(request) {
        ...
        request.signal?.throwIfAborted();
        if (!reply.ok)
            throw new ApiError(reply.status, code(reply.text));
        if (reply.status === 204 || reply.text === "")
            return undefined;
        return JSON.parse(reply.text);
    }
}
/** A refusal names its reason in the body; anything unreadable is simply a failed request. */
function code(body) {
    try {
