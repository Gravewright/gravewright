...
/** Three printed cells, spread from the upper left to the lower right (legacy mapper). */
export function calculateCalibration(samples) {
    ...
    if (samples.some(s => Math.abs(s.width - roughSize) > roughSize * .35 || Math.abs(s.height - roughSize) > roughSize * .35))
        throw new Error(gwText('Mark a single cell of similar size at each step.'));
    let size = (spanX + spanY) / (columns + rows);
    if (Math.abs(size - Math.round(size)) <= Math.max(.08, size * .0025))
        size = Math.round(size);
    const offset = (value) => { const n = ((value % size) + size) % size; return Math.min(n, size - n) <= Math.max(.75, size * .01) ? 0 : n; };
    return { size, offsetX: offset(origin.x), offsetY: offset(origin.y) };
}
export function calibrationSettings(value, imageScale) {
