...
class CombatStrategyService:
    ...
    def roll_participant_initiative(
        ...
        *,
        ...
        2. ``initiative.roll.actionId`` pointing to a system action such as
           ``roll.initiative``;
        3. legacy ``initiativeRoll`` / ``turnOrder.formula``;
        4. strategy-specific single result for deck/manual systems.
        """
        ...
        formula, source = self._initiative_formula(initiative=initiative, combat_config=combat_config, turn_order=turn_order, actor=actor)
        if formula:
    ...
    def _individual_initiative(
        ...
        *,
        ...
        for participant in participants:
            ...
            context = self._actor_context(actor=actor, campaign_id=campaign_id, token=token)
            formula, source = self._initiative_formula(initiative=initiative, combat_config={"initiative": initiative}, turn_order=turn_order, actor=actor)
            if not formula:
    ...
    def _initiative_formula(self, *, initiative: dict, combat_config: dict, turn_order: dict, actor: dict | None) -> tuple[str, dict]:
        legacy = combat_config.get("initiativeRoll") if isinstance(combat_config.get("initiativeRoll"), dict) else {}
        roll = initiative.get("roll") if isinstance(initiative.get("roll"), dict) else {}
        action_id = str(roll.get("actionId") or legacy.get("actionId") or "roll.initiative")
        label = initiative.get("label") or legacy.get("label") or turn_order.get("label")
        ...
        if legacy.get("formula"):
            return str(legacy["formula"]), {"kind": "initiative_roll", "label": label, "actionId": action_id}
        ...
        if turn_order.get("formula"):
            return str(turn_order["formula"]), {"kind": "turn_order", "strategy": turn_order.get("strategy")}
        return "", {"kind": "manual"}
    ...
    def _initiative_config(self, *, combat_config: dict, turn_order: dict) -> dict:
        ...
        if initiative:
            return initiative
        legacy = combat_config.get("initiativeRoll") if isinstance(combat_config.get("initiativeRoll"), dict) else {}
        roll = {"actionId": legacy.get("actionId") or "roll.initiative"}
        if legacy.get("formula"):
            roll["formula"] = legacy.get("formula")
        elif turn_order.get("formula"):
            roll["formula"] = turn_order.get("formula")
        return {
            "mode": self._mode_for_strategy(str(turn_order.get("strategy") or "manual")),
            "label": legacy.get("label") or turn_order.get("label") or "Iniciativa",
            "roll": roll,
            "sort": {
                "direction": legacy.get("sort") or turn_order.get("sort") or "desc",
                "tieBreakers": legacy.get("tieBreakers") or turn_order.get("tieBreakers") or [],
            },
            "allowReroll": bool(legacy.get("allowReroll", True)),
            "allowManualEdit": bool(legacy.get("allowManualEdit", True)),
        }
