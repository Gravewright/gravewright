...
class CombatConfig:
    ...
    @property
    def initiative_roll(self) -> dict:
        """Legacy payload kept for older frontend code and saved encounters."""
        return _legacy_initiative_roll(self.initiative, self.turn_order)
    ...
    def payload(self) -> dict:
        return {
            ...
            "initiativeRoll": self.initiative_roll,
...
class CombatConfigService:
    ...
    labels, icons, palette and skin classes for the combat panel. The older
    ``turnOrder`` and ``initiativeRoll`` fields are still normalized so existing
    systems keep working while new systems can define initiative modes,
    formulas/actions, sorting, tie-breakers and UI in one place.
    """
    ...
    def get_for_system(self, system_id: str | None) -> CombatConfig:
        ...
        ui = _normalize_ui(raw.get("ui"), initiative=initiative, legacy_roll=raw_legacy_roll)
...
def _normalize_initiative(raw: Any, turn_order: dict, legacy_roll: dict) -> dict:
    initiative = dict(raw) if isinstance(raw, dict) else {}
    mode = _initiative_mode(str(initiative.get("mode") or ""), turn_order)
    label = str(initiative.get("label") or legacy_roll.get("label") or turn_order.get("label") or "Iniciativa")
    ...
    if legacy_roll.get("actionId") and not roll.get("actionId"):
        roll["actionId"] = legacy_roll.get("actionId")
    if legacy_roll.get("formula") and not roll.get("formula"):
        roll["formula"] = legacy_roll.get("formula")
    if turn_order.get("formula") and not roll.get("formula") and _strategy_for_mode(mode) == "formula_sort":
        roll["formula"] = turn_order.get("formula")
    if not roll:
        roll = {"actionId": "roll.initiative"}

    sort = _normalize_sort(
        initiative.get("sort"),
        fallback_direction=initiative.get("sortDirection") or legacy_roll.get("sort") or turn_order.get("sort") or "desc",
        fallback_tie_breakers=initiative.get("tieBreakers") or legacy_roll.get("tieBreakers") or turn_order.get("tieBreakers") or [],
    )
    ...
    normalized = {
        ...
        "roll": roll,
        "sort": sort,
        "allowReroll": bool(initiative.get("allowReroll", legacy_roll.get("allowReroll", True))),
        "allowManualEdit": bool(initiative.get("allowManualEdit", legacy_roll.get("allowManualEdit", True))),
    }
    if isinstance(initiative.get("appearance"), dict):
        normalized["appearance"] = _normalize_legacy_appearance(initiative["appearance"])
    elif isinstance(legacy_roll.get("appearance"), dict):
        normalized["appearance"] = _normalize_legacy_appearance(legacy_roll["appearance"])
...
def _normalize_ui(raw: Any, *, initiative: dict, legacy_roll: dict) -> dict:
    ...
    legacy_appearance = initiative.get("appearance") if isinstance(initiative.get("appearance"), dict) else {}
    if not legacy_appearance and isinstance(legacy_roll.get("appearance"), dict):
        legacy_appearance = _normalize_legacy_appearance(legacy_roll["appearance"])

    normalized_combat: dict[str, Any] = {
        "skin": _safe_token(combat.get("skin") or combat.get("theme") or legacy_appearance.get("theme") or default_combat["skin"]),
        ...
        "initiative": _normalize_ui_initiative(combat.get("initiative"), initiative=initiative, legacy_appearance=legacy_appearance),
...
def _normalize_ui_initiative(raw: Any, *, initiative: dict, legacy_appearance: dict) -> dict:
    raw_initiative = raw if isinstance(raw, dict) else {}
    defaults = DEFAULT_COMBAT_UI["combat"]["initiative"]
    return {
        "icon": _safe_icon(raw_initiative.get("icon") or legacy_appearance.get("icon") or defaults["icon"]),
        "monsterIcon": _safe_icon(raw_initiative.get("monsterIcon") or legacy_appearance.get("monsterIcon") or defaults["monsterIcon"]),
        "rollAllLabel": _safe_label(raw_initiative.get("rollAllLabel") or legacy_appearance.get("rollAllLabel") or defaults["rollAllLabel"]),
        "rollMonstersLabel": _safe_label(raw_initiative.get("rollMonstersLabel") or legacy_appearance.get("rollMonstersLabel") or defaults["rollMonstersLabel"]),
        "rollSubtitle": _safe_label(raw_initiative.get("rollSubtitle") or legacy_appearance.get("rollSubtitle") or defaults["rollSubtitle"]),
        "monsterSubtitle": _safe_label(raw_initiative.get("monsterSubtitle") or legacy_appearance.get("monsterSubtitle") or defaults["monsterSubtitle"]),
        "scoreLabel": _safe_label(raw_initiative.get("scoreLabel") or initiative.get("label") or "Iniciativa"),
    }


def _normalize_legacy_appearance(raw: Any) -> dict:
    if not isinstance(raw, dict):
        return {}
    out: dict[str, Any] = {}
...
def _legacy_initiative_roll(initiative: dict, turn_order: dict) -> dict:
    roll = initiative.get("roll") if isinstance(initiative.get("roll"), dict) else {}
    sort = initiative.get("sort") if isinstance(initiative.get("sort"), dict) else {}
    legacy = {
        "actionId": str(roll.get("actionId") or "roll.initiative"),
        ...
        "tieBreakers": sort.get("tieBreakers") if isinstance(sort.get("tieBreakers"), list) else [],
        "allowReroll": bool(initiative.get("allowReroll", True)),
        "allowManualEdit": bool(initiative.get("allowManualEdit", True)),
    }
    if roll.get("formula"):
        legacy["formula"] = roll.get("formula")
    return legacy
