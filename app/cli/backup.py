...
snapshot, and (optionally) the ``storage/`` assets. Packages themselves are not
backed up — they are reinstallable from ``data/packages`` / the lockfile.
...
def create_backup(*, out: Path, include_assets: bool, verify: bool) -> tuple[int, dict]:
    src = _sqlite_source()
    if src is None:
