...
class LocalActorAssetStorage:
    """Stores actor images under ``storage/actor-assets/<campaign>/<actor>/<kind><ext>``.

    One image per actor per kind (portrait/token). Re-uploading overwrites it; a
    different extension replaces the previous variant so only one file per kind
    remains.
    """

    def __init__(self, *, root: Path | None = None) -> None:
        self.root = root or PROJECT_ROOT / "storage" / "actor-assets"
    ...
    def write_image(
        ...
        *,
        ...
        self._validate_id(actor_id, "actor_id")
